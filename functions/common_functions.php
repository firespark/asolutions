...
function custom_theme_setup(){
    ...
    load_theme_textdomain( 'asolutions', get_template_directory() . '/languages' );
    ...
    if(function_exists('acf_add_options_page')){
        acf_add_options_page([
            'page_title' => __('Main settings', 'asolutions'),
            'menu_title' => __('Main settings', 'asolutions'),
            'update_button'   => __('Save', 'asolutions'),
...
function custom_breadcrumbs($args = []) {
    ...
    /* === Options === */
    $text['home'] = __('Home', 'asolutions');
    $text['category'] = '%s';
    $text['search'] = __('Search results with query', 'asolutions') . ' "%s"';
    $text['tag'] = __('Posts with tag', 'asolutions') . ' "%s"';
    $text['author'] = __('Authors posts', 'asolutions') . ' %s';
    $text['404'] = __('Error', 'asolutions') . ' 404';
    $text['page'] = __('Page', 'asolutions') . ' %s';
    $text['cpage'] = __('Comments page', 'asolutions') . ' %s';

    $wrap_before = '<div class="crumbs"><div class="crumbs__container"><div class="crumbs__body">';
    $wrap_after = '</div></div></div>';
    $sep = ' | ';
    $before = '<span class="crumbs__link">';
    $after = '</span>';
...
function pagination_admin_panel($page, $paged, $get_par, $total_pages){
    $pagination = '<span>' . __('Pages', 'asolutions') . ': </span>';
    if ($paged !== 1){$pagination .= '<a href="/wp-admin/admin.php?page=' . $page . $get_par . '" title="' . __('First page', 'asolutions') . '">&lt;&lt;&lt;</a>';}
    ...
    if($paged !== $total_pages){
        $pagination .= '<a href="/wp-admin/admin.php?page=' . $page . '&paged=' . $total_pages . $get_par . '" title="' . __('Last page', 'asolutions') . '">&gt;&gt;&gt;</a>';
    }
    return $pagination;
}